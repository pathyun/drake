...
from robobee_plant_aero_disturbance import *
...
w0[0]=0#2.# 2.0 #-0.1 # 0.1 #0;
w0[1]=0#-2.# -2.#10.5 # 0  #-0.1;s
w0[2]=0#1. #.1 # 0.1 # 0.2;
...
robobee_plant_aero = RobobeePlantAero(
    ...
    g = g, rw_l= rw_l, bw=bw, bw_bound=bw_gap, input_max = input_max)
...
def test_Feedback_Linearization_controller_intermediate_pt(x,t):
    ...
    Iyy=I[1,1]
    Izz=I[2,2]
    U_fl[1:4] = U_fl[1:4]-k_yaw*w[2]*e3#-(Ixx-Iyy)*e3*w[0]*w[1]-Izz*w[2]*e3#-(Ixx-Iyy)*e3*w[0]*w[1]# -0.05*w
    # u[0] = g
    # if u[0]<0:
    #     u =np.zeros(4)
    ...
    prog.AddConstraint(c_var[0]<=1000)
    prog.AddConstraint(u_var[0]<=40)
    prog.AddConstraint(u_var[0]>=0)
    prog.AddConstraint(u_var[1]<=4)
    prog.AddConstraint(u_var[2]<=4)
    prog.AddConstraint(u_var[1]>=-4)
    prog.AddConstraint(u_var[2]>=-4)
