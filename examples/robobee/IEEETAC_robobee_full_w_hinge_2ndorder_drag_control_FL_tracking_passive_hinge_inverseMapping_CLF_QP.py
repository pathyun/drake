...
theta0 = 0.*math.pi/8.;  # angle of rotation
# v0_q=np.array([1.,0.,1.])
v0_q=np.array([0.,0.,1.])
...
def test_Feedback_Linearization_controller_intermediate_pt(x,t):
    ...
    if num_iter<filter_window_size:
        ...
        tau_aero_filtered[:,filter_window_size-1]=w
...
for j in range(0,num_iteration):
    ...
    Phi_amp_l = -(Phi_amp_avg+Phi_amp_diff)*gain_amp#-math.pi/8.5-math.pi/8.5*(zd-r_filtered_avg[2])*0.+math.pi/8.5*(v_filtered_avg[2])*0+math.pi/8.5*(w_filtered_avg[0])*0.05;
    Phi_amp_r = (Phi_amp_avg-Phi_amp_diff)*gain_amp#math.pi/8.5+math.pi/8.5*(zd-r_filtered_avg[2])*0.-math.pi/8.5*(v_filtered_avg[2])*0+math.pi/8.5*(w_filtered_avg[0])*0.05;
