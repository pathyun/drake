...
from robobee_plant_full_aero import *
...
x0= np.hstack([r0, q0, phi_default, -phi_default, v0, w0, 0, 0])
print("x0:", x0)
...
xf= np.hstack([rf, qf, phi_default, phi_default, vf, wf, 0, 0]) # Fixed point for the state
print("xf:", xf)
uf = np.hstack([uf_T,tau,0,0])  # Hovering
print("uf:",uf)
...
def test_Feedback_Linearization_controller_intermediate_pt(x,t):
    ...
    # Wing trajectory
    u[4] =  math.pi/4*w_resonance*math.cos(w_resonance*t)
    u[5] = -math.pi/4*w_resonance*math.cos(w_resonance*t)
    # u[0] = g
    # if u[0]<0:
    #     u =np.zeros(4)
    U_fl = u;
