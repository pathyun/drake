...
from robobee_plant_full_w_hinge_aero import *
...
print("xf:", xf)
uf = np.hstack([uf_T,tau,0,0,0,0])  # Hovering
print("uf:",uf)
...
def test_harmonic_controller(x, t):
    ...
    # [-pi, pi].
    u = np.zeros(8)
    global g, m, I, r_w, rw_l, bw, xf, Aout, Bout, Mout, T_period, w_freq, radius, ratio_ab, delta_lissajous, ratio_rw, circle_flag, w_resonance, u_sampled, rho, beta_geometry, CL, chordlength_r, chordlength_cr, blade_width
    ...
    mean_stroke_l = -math.pi/10*math.sin(w_resonance*0.05*t);
    mean_stroke_r = +math.pi/10*math.sin(w_resonance*0.05*t);
    ...
    if torque_flag==1:
        ...
        torque_async =math.pi;
    else:
        # Slow varing mean stroke angle
        mean_stroke_l = 0 #-math.pi/5*math.sin(w_resonance*0.05*t);
        mean_stroke_r = 0 #math.pi/5*math.sin(w_resonance*0.05*t);
        half_wave_weight = 0.0;
        half_wave_freq = 0.5*w_resonance;
        torque_async =0.0;

    dphi_l = decay_rate_wing*(phi_l-mean_stroke_l)-math.pi/3*math.sqrt(math.pow(w_resonance,2)+math.pow(decay_rate_wing,2))*(half_wave_weight*math.sin(half_wave_freq*t)+1.*math.sin(w_resonance*t))
    dphi_r = decay_rate_wing*(phi_r-mean_stroke_r)+math.pi/3*math.sqrt(math.pow(w_resonance,2)+math.pow(decay_rate_wing,2))*(half_wave_weight*math.sin(half_wave_freq*t+torque_async)+math.sin(w_resonance*t+torque_async))
    dpsi_l = math.pi/3.*math.pow(w_resonance,1)*math.cos(w_resonance*t)
    dpsi_r = math.pi/3.*math.pow(w_resonance,1)*math.cos(w_resonance*t+torque_async)
    ...
    for i_iter in range(0,num_blade):
        ...
        FL_r_r[i_iter] = 1./2.*rho*CL_r*v_ortho_l_r_norm_sq*chordlength_cr[i_iter]*blade_width
...
def test_Feedback_Linearization_controller_intermediate_pt(x,t):
    ...
    global g, m, I, r_w, rw_l, bw, xf, Aout, Bout, Mout, T_period, w_freq, radius, ratio_ab, delta_lissajous, ratio_rw, circle_flag, w_resonance, u_sampled
    ...
    decay_rate_wing = -1e2;
    mean_stroke_r = -math.pi/4;
    mean_stroke_l = math.pi/4;
    
    # if truncate(t*1000,2)%80>39:
    #     mean_stroke_r = -math.pi/4;
    #     mean_stroke_l = -math.pi/4;
    # else:
    #     mean_stroke_r = math.pi/4;
    ...
    # Slow varing mean stroke angle
    mean_stroke_l = -math.pi/4*math.sin(w_resonance*0.05*t);
    mean_stroke_r = +math.pi/4*math.sin(w_resonance*0.05*t);
    ...
    if torque_flag==1:
        ...
        # Slow varing mean stroke angle
        mean_stroke_l = 0 #-math.pi/4#-math.pi/4*math.sin(w_resonance*0.05*t);
        mean_stroke_r = 0 #math.pi/4 #math.pi/4*math.sin(w_resonance*0.05*t);
        half_wave_weight = 0.0;
        half_wave_freq = 0.5*w_resonance;
        torque_async =0.0;
    u[4] = decay_rate_wing*(phi_l-mean_stroke_l)-math.pi/4*math.sqrt(math.pow(w_resonance,2)+math.pow(decay_rate_wing,2))*(half_wave_weight*math.sin(half_wave_freq*t)+1.*math.sin(w_resonance*t))
    u[5] = decay_rate_wing*(phi_r-mean_stroke_r)+math.pi/4*math.sqrt(math.pow(w_resonance,2)+math.pow(decay_rate_wing,2))*(half_wave_weight*math.sin(half_wave_freq*t+torque_async)+math.sin(w_resonance*t+torque_async))

    u[6] = math.pi/3*math.pow(w_resonance,1)*math.cos(w_resonance*t)
    u[7] = math.pi/3*math.pow(w_resonance,1)*math.cos(w_resonance*t+torque_async)
    print("u4:", u[4])
    print("phi_l:", phi_l)
    # u[0] = g
    # if u[0]<0:
    ...
    if vicon_sampled_time==0:
        u_sampled = u
    
    u_final = u_sampled
    print("u final:", u_final)
