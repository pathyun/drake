...
theta0 = 1.*math.pi/8.;  # angle of rotation
# v0_q=np.array([1.,0.,1.])
v0_q=np.array([1.,0.,1.])
...
for j in range(0,num_iteration):
    ...
    if num_iter<filter_window_size:
        ...
        w_eig, vr_eig = np.linalg.eig(M_mat_quat)
        # print("vr_eig:", vr_eig)
        q_filtered_avg_out[:,j] = vr_eig[:, np.argmax(w_eig)]
    # Averaging filter
    ...
    Phi_amp_l = -math.pi/8.5;
    Phi_amp_r = math.pi/8.5;
    ...
    Phi_amp_sq = ((F_l_avg[j]+F_r_avg[j])*r_cp*math.cos(Phi_off_l_estimate)+F_l_avg_coeff*(tau_aero_r_avg[0,j]+tau_aero_l_avg[0,j]))/(r_cp*math.cos(Phi_off_l_estimate)*F_l_avg_coeff)/2.
    Phi_amp_sq_cross = ((F_l_avg[j]+F_r_avg[j])*r_cp*math.cos(Phi_off_l_estimate)-F_l_avg_coeff*(tau_aero_r_avg[0,j]+tau_aero_l_avg[0,j]))/(r_cp*math.cos(Phi_off_l_estimate)*F_l_avg_coeff)/2.
    ...
    if show_flag_control==1:
        ...
        for i in range(0,6):
            ...
            if i==0:
                ...
                compare2= Thrust
                compare3 = Thrust_r_avg+Thrust_l_avg
                # plt.plot(state_log.sample_times()*time_gain, compare2, color='green')

                plt.plot(state_log.sample_times()*time_gain, compare3, color='green')
            elif i==1:
                mg_gain=1e0;
