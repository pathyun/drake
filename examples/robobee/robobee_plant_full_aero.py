...
class RobobeePlantAero(VectorSystem):
    def __init__(self, m = 1., Ixx = 1., 
        ...
        VectorSystem.__init__(self,
            6,                           # One input (torque at reaction wheel).
            17)                           # Four outputs (theta, phi, dtheta, dphi)
        self._DeclareContinuousState(17)  # Four states (theta, phi, dtheta, dphi).
    ...
    def evaluate_f(self, u, x, throw_when_limits_exceeded=True):
        ...
        return np.hstack([rd, qd, dphid_l, dphid_r, vd, wd, ddphid_l, ddphid_r])
...
class RobobeeController(VectorSystem):
    ...
    def __init__(self, feedback_rule):
        VectorSystem.__init__(self,
            ...
            6)                           # One output (torque for reaction wheel).
...
class RigidBodySelection(VectorSystem):
    ...
    def __init__(self):
        VectorSystem.__init__(self,
            17,                           # Four inputs: full state inertial wheel pendulum..
    ...
    def _DoCalcVectorOutput(self, context, u, x, y):
        # Remember that the input "u" of the controller is the
        # state of the plant
        y[:] = np.hstack([u[0:7],u[9:15]])

def RunSimulation(robobee_plantBS_torque, control_law, x0=np.random.random((17, 1)), duration=30):
    ...
    state_log = builder.AddSystem(SignalLogger(17))
