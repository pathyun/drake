...
# 30 blades
chordlength_r = np.loadtxt('chordlength_list_r_30.txt')
chordlength_cr = np.loadtxt('chordlength_list_cr_30.txt')
chordlength_yLE = np.loadtxt('chordlength_list_yLE_30.txt')
...
gain_amp_default = 0.95; #0.82
gain_offset_default=4. #4; #3
gain_diff_default=.6; #0.3

gain_amp = gain_amp_default
gain_offset= gain_offset_default
gain_diff = gain_diff_default
...
rf = np.array([-0.15,0.15,0.3])+rw_l*e3
...
def test_harmonic_controller(x, t):
    ...
    if torque_flag==1:
        ...
        # Slow varing mean stroke angle
        mean_stroke_l = 0 #-math.pi/15*math.sin(w_resonance*0.1*t);
        mean_stroke_r = math.pi/4 # math.pi/15*math.sin(w_resonance*0.1*t);
        half_wave_weight = 0.0;
        wave_weight = 1.;
    ...
    Phi_amp_l = 0 #-math.pi/8.5#-math.pi/8.5*(zd-r_filtered_avg[2])*0.+math.pi/8.5*(v_filtered_avg[2])*0+math.pi/8.5*(w_filtered_avg[0])*0.05;
...
def test_Feedback_Linearization_controller_intermediate_pt(x,t):
    ...
    # print("r_shape:", r.shape)
    if num_iter<filter_window_size:
        r_filtered=np.matmul(r_instant.reshape(3,1),np.ones((1,filter_window_size)))
        q_filtered=np.dot(q_quat_instant.reshape(4,1),np.ones((1,filter_window_size)))
        v_filtered=np.dot(v_instant.reshape(3,1),np.ones((1,filter_window_size)))
        w_filtered=np.dot(w_instant.reshape(3,1),np.ones((1,filter_window_size)))

        q_filtered_avg = q_quat_instant
    else:
        r_filtered[:,0:filter_window_size-1]=r_filtered[:,1:filter_window_size]
        q_filtered[:,0:filter_window_size-1]=q_filtered[:,1:filter_window_size]
        v_filtered[:,0:filter_window_size-1]=v_filtered[:,1:filter_window_size]
        ...
        r_filtered[:,filter_window_size-1]=r_instant
        q_filtered[:,filter_window_size-1]=q_quat_instant
        v_filtered[:,filter_window_size-1]=v_instant
        w_filtered[:,filter_window_size-1]=w_instant
    ...
    u_ForceTorque[1:3]=u_ForceTorque[1:3]#-np.array([Ixx*w[0],Iyy*w[1]])
    ...
    ### Instantaneous force calculation
    r=r_instant
    v=v_instant
    w=w_instant
    q_quat=q_quat_instant

    (Rq, Eq, wIw, I_inv, fdw, taudw, vd_aero, wd_aero)=robobee_plant_aero.GetManipulatorDynamics(np.hstack([r, q_quat]), np.hstack([v, w]))
    ...
    for i_iter in range(0,num_blade):
        ...
        if v_ortho_l_r_norm==0 or v_ortho_r_r_norm==0 :
            v_ortho_l_r_normalized = np.zeros(3)
            v_ortho_r_r_normalized = np.zeros(3)
            # v_ortho_l_r_normalized_wind = np.zeros(3)
            # v_ortho_r_r_normalized_wind = np.zeros(3)
        else:
            v_ortho_l_r_normalized = v_ortho_l_r/v_ortho_l_r_norm
            v_ortho_r_r_normalized = v_ortho_r_r/v_ortho_r_r_norm
         # # Drag proportional to the relativ veloicy
         #    deta_l = deta + (Rqw_hatRq_le2+ RqRq_ldphi_l_hate2)*chord_r_from_root 
         #    deta_r = deta - (Rqw_hatRq_re2+ RqRq_rdphi_r_hate2)*chord_r_from_root

         #    v_ortho_l_r_normalized_wind = deta_l/math.sqrt(np.dot(deta_l,deta_l))
         #    v_ortho_r_r_normalized_wind = deta_l/math.sqrt(np.dot(deta_r,deta_r)) 
        ...
        # FD_l_r[0:3,i_iter] = -1./2.*rho*CD_l*v_ortho_l_r_norm_sq*v_ortho_l_r_normalized_wind*chordlength_cr[i_iter]*blade_width
        # FD_r_r[0:3,i_iter] = -1./2.*rho*CD_r*v_ortho_r_r_norm_sq*v_ortho_r_r_normalized_wind*chordlength_cr[i_iter]*blade_width
    ...
    FL_l = np.sum(FL_l_r)
    FL_r = np.sum(FL_r_r)
    FD_l = np.sum(FD_l_r,axis=1)
    FD_r = np.sum(FD_r_r,axis=1)
    ...
    # gain_offset = gain_offset -0.001*(tau_aero_filtered_avg[1]-torque_desired[1])
    # gain_diff = gain_diff -0.0001*(tau_aero_filtered_avg[0]-torque_desired[0])
    # gain_amp = gain_offset -(tau_aero_filtered_avg-torque_desired[1])
...
taudw_all=np.zeros((3,num_iteration))
...
FD_total=np.zeros((3,num_iteration))

tau_Drag_sum=np.zeros((3,num_iteration))
tau_Drag_r=np.zeros((3,num_iteration))
tau_Lift_sum=np.zeros((3,num_iteration))
tau_Lift_r=np.zeros((3,num_iteration))
...
for j in range(0,num_iteration):
    ...
    if num_iter<filter_window_size:
        ...
        q_filtered_avg_out[:,j] = q_quat_instant
    else:
        r_filtered[:,0:filter_window_size-1]=r_filtered[:,1:filter_window_size]
        q_filtered[:,0:filter_window_size-1]=q_filtered[:,1:filter_window_size]
        v_filtered[:,0:filter_window_size-1]=v_filtered[:,1:filter_window_size]
    ...
    r=r_filtered_avg_out[:,j]
    v=v_filtered_avg_out[:,j]
    w=w_filtered_avg_out[:,j]
    q0=q_filtered_avg_out[0,j]
    q1=q_filtered_avg_out[1,j]
    q2=q_filtered_avg_out[2,j]
    q3=q_filtered_avg_out[3,j]
    ...
    (Rq, Eq, wIw, I_inv, fdw, taudw, vd_aero, wd_aero)=robobee_plant_aero.GetManipulatorDynamics(np.hstack([r_filtered_avg_out[:,j], q_filtered_avg_prev_out[0:4,j]]), np.hstack([v_filtered_avg_out[:,j], w_filtered_avg_out[:,j]]))
    ...
    fdw=np.dot(BW,fdw)
    RqTfdw=np.dot(Rq.T,fdw)
    taudw_all[0:3,j] = np.cross(r_w,RqTfdw)
        
    vd_aero =  np.dot((-g*np.eye(3)),e3) + fdw/m # \dot{v} = -ge3 +R(q)e3 u[0] : u[0] Thrust is a unit of acceleration
    wd_aero = -np.dot(I_inv,wIw)+ np.dot(I_inv, taudw_all[0:3,j])
    ...
    tau_aero_l =np.zeros((3,num_blade))
    tau_aero_r =np.zeros((3,num_blade))
    tau_Drag_l =np.zeros((3,num_blade))
    tau_Drag_r =np.zeros((3,num_blade))
    tau_Lift_l =np.zeros((3,num_blade))
    tau_Lift_r =np.zeros((3,num_blade))
    ...
    for i_iter in range(0,num_blade):
        ...
        tau_Drag_l[0:3,i_iter] = np.cross(chord_r_CoP_world_l[0:3,i_iter],FD_l_vec[0:3,i_iter])
        tau_Drag_l[0:3,i_iter] = np.dot(Rq.T,tau_Drag_l[0:3,i_iter])

        tau_Drag_r[0:3,i_iter] = np.cross(chord_r_CoP_world_r[0:3,i_iter],FD_r_vec[0:3,i_iter])
        tau_Drag_r[0:3,i_iter] = np.dot(Rq.T,tau_Drag_r[0:3,i_iter])

        tau_Lift_l[0:3,i_iter] = np.cross(chord_r_CoP_world_l[0:3,i_iter],FL_l_vec[0:3,i_iter])
        tau_Lift_l[0:3,i_iter] = np.dot(Rq.T,tau_Lift_l[0:3,i_iter])

        tau_Lift_r[0:3,i_iter] = np.cross(chord_r_CoP_world_r[0:3,i_iter],FL_r_vec[0:3,i_iter])
        tau_Lift_r[0:3,i_iter] = np.dot(Rq.T,tau_Lift_r[0:3,i_iter])
    ...
    FD_total[0:3,j]=(FD_l+FD_r)/Force_scale

    # print("FD_total:", FD_total[:,j])
    # print("Thrust left:", Thrust_l)
    # print("Thrust right:", Thrust_r)
    # print("Thrust:", Thrust)
    ...
    tau_Drag_l_sum = np.sum(tau_Drag_l, axis=1)
    tau_Drag_r_sum = np.sum(tau_Drag_r, axis=1)
    tau_Drag_sum[0:3,j] = tau_Drag_l_sum+tau_Drag_r_sum

    tau_Lift_l_sum = np.sum(tau_Lift_l, axis=1)
    tau_Lift_r_sum = np.sum(tau_Lift_r, axis=1)
    tau_Lift_sum[0:3,j] = tau_Lift_l_sum+tau_Lift_r_sum
    # print("tau_aero_l: ", tau_aero_l_sum)
    # print("tau_aero_r: ", tau_aero_r_sum)
    # print("tau_aero: ", tau_aero)
    # print("tau_aero_sum-all", tau_aero_sum[0:3,j]-tau_Drag_sum[0:3,j]-tau_Lift_sum[0:3,j])
...
tau_Drag_roll_filtered=np.convolve(tau_Drag_sum[0,:], filter_num, mode='same')
tau_Drag_pitch_filtered=np.convolve(tau_Drag_sum[1,:], filter_num, mode='same')
tau_Drag_yaw_filtered=np.convolve(tau_Drag_sum[2,:], filter_num, mode='same')

tau_Lift_roll_filtered=np.convolve(tau_Lift_sum[0,:], filter_num, mode='same')
tau_Lift_pitch_filtered=np.convolve(tau_Lift_sum[1,:], filter_num, mode='same')
tau_Lift_yaw_filtered=np.convolve(tau_Lift_sum[2,:], filter_num, mode='same')

FD_filtered_x =np.convolve(FD_total[0,:], filter_num, mode='same')
FD_filtered_y =np.convolve(FD_total[1,:], filter_num, mode='same')
FD_filtered_z =np.convolve(FD_total[2,:], filter_num, mode='same')

fdw_all_filtered_x =np.convolve(fdw_all[0,:], filter_num, mode='same')
fdw_all_filtered_y =np.convolve(fdw_all[1,:], filter_num, mode='same')
fdw_all_filtered_z =np.convolve(fdw_all[2,:], filter_num, mode='same')
    ...
    if show_flag_control==1:
        ...
        for i in range(0,6):
            ...
            if i==0:
                ...
                compare = u_all[0,:] #Thrust_r_avg+Thrust_l_avg;
                compare2= Thrust
                ...
                compare = u_all[1,:] #tau_aero_l_avg[0,:]+tau_aero_r_avg[0,:];
                compare2 = tau_roll_filtered;
                compare3 = tau_Drag_roll_filtered;
                compare4 = taudw_all[0,:];
                compare5 = tau_Lift_roll_filtered;
                compare6 = u_all[1,:]+taudw_all[0,:]; #tau_Drag_roll_filtered+tau_Lift_roll_filtered;
                plt.plot(state_log.sample_times()*time_gain, compare2, color='red', linestyle='--')
                plt.plot(state_log.sample_times()*time_gain, compare3, color='black', linestyle='--')
                plt.plot(state_log.sample_times()*time_gain, compare4, color='black')
                plt.plot(state_log.sample_times()*time_gain, compare5, color='green', linestyle='--')
                plt.plot(state_log.sample_times()*time_gain, compare6, color='red')
           
            elif i==2:
                mg_gain=1e0;
                control = mg_gain*ubar[i+2,:];
                compare = u_all[2,:] #tau_aero_l_avg[1,:]+tau_aero_r_avg[1,:];
                compare2 = tau_yaw_filtered;
                compare3 = tau_Drag_pitch_filtered;
                compare4 = taudw_all[1,:];
                compare5 = tau_Lift_pitch_filtered;
                compare6 = u_all[2,:]+taudw_all[1,:]; #tau_Drag_pitch_filtered+tau_Lift_pitch_filtered;
                plt.plot(state_log.sample_times()*time_gain, compare2, color='red', linestyle='--')
                plt.plot(state_log.sample_times()*time_gain, compare3, color='black', linestyle='--')
                plt.plot(state_log.sample_times()*time_gain, compare4, color='black')
                plt.plot(state_log.sample_times()*time_gain, compare5, color='green', linestyle='--')
                ...
                mg_gain=1e0;
                control = mg_gain*ubar[i+2,:];
                compare = u_all[3,:] #tau_aero_l_avg[2,:]+tau_aero_r_avg[2,:];
                compare3 = tau_Drag_yaw_filtered;
                compare4 = taudw_all[2,:];
                compare5 = tau_Lift_yaw_filtered;
                plt.plot(state_log.sample_times()*time_gain, compare3, color='black')
                plt.plot(state_log.sample_times()*time_gain, compare4, color='magenta')
                plt.plot(state_log.sample_times()*time_gain, compare5, color='green', linestyle='--')
            else:
        ...
        fig = plt.figure(500).set_size_inches(6, 6)
        for i in range(0,3):
            # print("i:%d" %i)
            plt.subplot(3, 1, i+1)
            # print("test:", num_state)
            if i==0:
                control = FD_filtered_x;
                compare = fdw_all[0,:];
                plt.plot(state_log.sample_times()*time_gain, compare, color='red', linestyle='-')
                plt.plot(state_log.sample_times()*time_gain, control)
                
            elif i==1:
                control = FD_filtered_y;
                compare = fdw_all[1,:];
                plt.plot(state_log.sample_times()*time_gain, compare, color='red', linestyle='-')
                plt.plot(state_log.sample_times()*time_gain, control)
           
            elif i==2:
                control = FD_filtered_z;
                compare = fdw_all[2,:];
                plt.plot(state_log.sample_times()*time_gain, compare, color='red', linestyle='-')
                plt.plot(state_log.sample_times()*time_gain, control)
            ...
            if i==0:
                ...
                plt.ylabel("Drag x (mN)")
            elif i==1:
                plt.ylabel("Drag y (mN)")
            elif i==2:
                plt.ylabel("Drag z (mN)")
            elif i==3:
                plt.ylabel(r"$\tau_3$ (mNmm)")
            elif i==4:
                plt.ylabel(r"$V_l$ (V)")
